...
public class GetCarListAsyncTask extends BaseAsyncTask{
     ...
     * @brief コンストラクタ
     */
    public GetCarListAsyncTask(Context context, AsyncTaskCallbacks callback) {
     ...
     * @return
     */
    private int set_car_list( String body ){
        int         ret             = ErrorCode.STS_OK;
        long        ins_ret         = 0;
        String      sql             = "";
        String      list            = "{";

        MtbTrucks   mtb_trucks  = new MtbTrucks( this.context );
        ...
        try {
            ...
            JSONObject  json_truck      = json_response.getJSONObject( 0 );
            if( !json_truck.has( "Truck" ) ){
                return ErrorCode.STS_NG_NOT_FOUND_DATA;
            }
            JSONObject  json_trucks     = json_truck.getJSONObject( "Truck" );

            //値リストの設定
            for (int idx = 0; idx < json_trucks.length(); idx++) {
                //1件ずつ処理する
                JSONObject		rec		= json_trucks.getJSONObject( String.valueOf( idx + 1 ) );

                list    += "\"" + rec.getString( "id" ) +  "\":\"" + rec.getString( "license_plate" ) + "\",";
                sql     = mtb_trucks.getInsertStatement( rec );
                ins_ret = mtb_trucks.insert( sql );
            }
            //末尾がカンマの場合は、カンマを削除
            if( list.endsWith( "," ) ) {
