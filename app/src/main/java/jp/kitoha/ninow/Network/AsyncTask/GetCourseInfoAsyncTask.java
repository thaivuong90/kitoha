...
public class GetCourseInfoAsyncTask extends BaseAsyncTask {
	 ...
	 * @brief コンストラクタ
	 */
	public GetCourseInfoAsyncTask( Context context, AsyncTaskCallbacks callback ) {
	...
	protected int pre_proc( String... params ){
		...
		if( params.length == 0 ) {
			...
			acquire_flag			= Integer.parseInt( params[6] );
		}
	...
	protected int main_proc(){
		...
		//実行
		ret = command.get_course_info( delivery_date, acquire_flag, latitude, longitude, accuracy );
	...
	private int setDeliveryList( String body ){
		int         ret     		= ErrorCode.STS_OK;
		int         idx             = 0;                        //処理件数(伝票枚数)
		int         delivery_num    = 0;                        //配送先件数
		String		instruct_no		= "";						//配送指示番号
		String      sql     		= "";
		String      list    		= "[{";
		//配送先カウント時の比較項目
		String		pre_consignee_address	= "";
		String		pre_consignee_company	= "";
		String		pre_consignee_section	= "";
		String		pre_consignee_contact	= "";
		...
		try {
			...
			//値リストの設定
			for( idx = 0; idx < json_orders.length(); idx++ ){
				//1件ずつ処理する.
				JSONObject	rec		= json_orders.getJSONObject( String.valueOf( idx + 1 ) );
				//配送指示番号の取得
				instruct_no			= rec.getString( "instruction_no" );

				//配送先件数のカウント
				if( ( rec.getString( "consignee_address" )		!= pre_consignee_address ) ||
					( rec.getString( "consignee_name" )			!= pre_consignee_company ) ||
					( rec.getString( "consignee_section_name" )	!= pre_consignee_section ) ||
					( rec.getString( "consignee_contact" )		!= pre_consignee_contact ) ){
					//件数を増やす
					delivery_num++;
					//新しい配送先を記録する
					pre_consignee_address	= rec.getString( "consignee_address" );
					pre_consignee_company	= rec.getString( "consignee_name" );
					pre_consignee_section	= rec.getString( "consignee_section_name" );
					pre_consignee_contact	= rec.getString( "consignee_contact" );
				}
...
//						+ "\"storage_id\":\"" + ( rec.isNull( "storage_id" ) ? 0 : rec.getInt( "storage_id" ) ) + "\","
			...
			run_info.setInstructNo( instruct_no );
			run_info.setVoucherNum( idx );
			run_info.setDeliveryNum( delivery_num );
			run_info.save();
