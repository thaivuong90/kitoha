 ...
 ******************************************************************************/
public class PostWorkEndAsyncTask extends BaseAsyncTask {
	/***************************************************************************
	 * インスタンス変数
	 ***************************************************************************/
	String			current_date;
	 ...
	 * @brief コンストラクタ
	 */
	public PostWorkEndAsyncTask( Context context, AsyncTaskCallbacks callback ) {
		super( context, callback );
		this.request_code	= RequestCode.REQ_CODE_POST_WORK_END;
	...
	protected int main_proc(){
		...
		run_info.setWorkEndTime( report_time );
		run_info.save();

		//実行
		ret = command.post_work_report( HttpCommand.TYPE_NO_END, latitude, longitude, accuracy, current_date, report_time );
