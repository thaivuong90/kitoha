 ...
 ******************************************************************************/
public class PostWorkStartAsyncTask extends BaseAsyncTask {
	/***************************************************************************
	 * インスタンス変数
	 ***************************************************************************/
	String			current_date;
	 ...
	 * @brief コンストラクタ
	 */
	public PostWorkStartAsyncTask( Context context, AsyncTaskCallbacks callback ) {
		super( context, callback );
		this.request_code	= RequestCode.REQ_CODE_POST_WORK_START;
	...
	protected int main_proc(){
		...
		if( !run_info.getCurrentDate().isEmpty() ) {
			current_date	= run_info.getCurrentDate();
		}

		//報告情報を設定
		report_time			= Utility.getNow();
		run_info.setWorkStartTime( report_time );
		run_info.save();

		//実行
		ret = command.post_work_report( HttpCommand.TYPE_NO_START, latitude, longitude, accuracy, current_date, report_time );
